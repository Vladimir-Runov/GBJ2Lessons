public class Cat implements Actiity {
    ...
    public boolean doRun(int distnce) {
        if (distnce > maxDistanceRun) {
            System.out.println("done: Кот. (" + this.name + ") НЕ пробежал(а) " + distnce + ",(max= " + maxDistanceRun + ")");
            return false;
        }
        System.out.println("done: Кот. ("+this.name+") пробежал(а)      " + distnce);
        return true;
    }

    public boolean doJump(int Height) {
        if (Height > maxHeight) {
            System.out.println("done: Кот. (" + this.name + ") не перепрыгнул(а) " + Height + ",(max= " + maxHeight + ")");
            return false;
        }
        System.out.println("done: Кот ("+this.name+") перепрыгнул(а) " + Height);
        return true;
    }
}
